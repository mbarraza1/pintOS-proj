...
struct sleeping_thread {
  ...
  int64_t wake_tick; // Tick when the thread should wake.
  struct list_elem elem;
};
...
struct thread_shared {
  ...
  struct semaphore sema;      /* semaphore for joining/exiting */
  struct semaphore data_sync; /* semaphore for synchronization of data */
  struct list_elem elem;      /*list element*/
  int ref_count;
  struct lock ref_lock;
  bool has_joined;
