...
/* List of all processes in THREAD_READY state that is to be used by the
...
tid_t thread_create(const char* name, int priority, thread_func* function, void* aux) {
  ...
  uint8_t fpu_prev[108];

  asm("fsave (%0); fninit; fsave (%1); frstor (%0)" : : "g"(&fpu_prev), "g"(&sf->fpu_state));

  /* Add to run queue. */
  thread_unblock(t);

  /* Preempt the current thread if the new thread has higher priority. */
...
static void thread_enqueue(struct thread* t) {
  ...
  if (active_sched_policy == SCHED_FIFO)
    ...
    /* Ordered insert of thread based on effective priority */
    list_insert_ordered(&priority_ready_list, &t->elem, priority_less_func, NULL);
  } else
    PANIC("Unimplemented scheduling policy value: %d", active_sched_policy);
}

/* Priority comparator function for list insert ordered Pintos */
bool priority_less_func(const struct list_elem* a, const struct list_elem* b, void* aux UNUSED) {
...
void thread_set_priority(int new_priority) {
  ...
  // Update effective priority and donation chain
...
void thread_update_effective_priority(struct thread* t) {
  //disable interrupts when setting priority
  /* Start with base priority */
  t->effective_priority = t->priority;

  /* Effective priority of each thread will be the highest effective priority of its held locks*/
  /* Check waiters of all held locks */
  if (!list_empty(&t->locks_held)) {
    for (struct list_elem* e = list_begin(&t->locks_held); e != list_end(&t->locks_held);
         e = list_next(e)) {
      ...
      if (!list_empty(&lock->semaphore.waiters)) {
        ...
        struct list_elem* w;
        for (w = list_begin(&lock->semaphore.waiters); w != list_end(&lock->semaphore.waiters);
             w = list_next(w)) {
          struct thread* waiter = list_entry(w, struct thread, elem);
          if (waiter->effective_priority > highest_waiter->effective_priority)
            highest_waiter = waiter;
        }
        if (highest_waiter->effective_priority > t->effective_priority)
          t->effective_priority = highest_waiter->effective_priority;
      }
    }
  }
  /* Propagate priority up the chain if we're waiting on a lock */
  if (t->waiting_on_lock != NULL && t->waiting_on_lock->holder != NULL) {
    ...
    if (t->effective_priority > holder->effective_priority) {
      ...
      thread_update_effective_priority(holder); // Recursively update the chain
    }
  }

  /* Yield if we are not the highest priority thread anymore */
  /*
  if (!intr_context() && thread_current() == t && active_sched_policy == SCHED_PRIO) {
    if (!list_empty(&priority_ready_list)) {
      struct thread *highest_ready = list_entry(list_back(&priority_ready_list), struct thread, elem);
      if (highest_ready->effective_priority > t->effective_priority) {
        thread_yield();
      }
    }
  }
...
/* Strict priority scheduler */
static struct thread* thread_schedule_prio(void) {
  /* Pop from the back because we have inserted in terms of decreasing priority */
  if (!list_empty(&priority_ready_list)) {
...
bool sleeping_thread_cmp(const struct list_elem* a, const struct list_elem* b, void* aux UNUSED) {
